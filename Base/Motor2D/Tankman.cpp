		...
		if (lead)
		{
			...
			case SEARCH:
				...
				if (IsInAllyZone(map_pos))
				{
					if (fromPlayer1)
					{
						info.closest = FindEntityInAttackRange(map_pos, fromPlayer1, original_range, entityType::WALLS);
						...
						{
							// soldiers in their zones being defensive mode
					...
					else
					{
						...
						{
							//en zona enemiga buscando que atacar
							info.closest = FindNearestEntity(map_pos, fromPlayer1, entityType::WALLS, 1);
							state = MOVING;
							pathfind = true;
						}
						else
				...
				else if (IsInEnemyZone(map_pos))
				{
					if (fromPlayer1)
					{
						...
						{
							//soldiers in eney zone tryng to make damage
							info.closest = FindNearestEntity(map_pos, fromPlayer1, entityType::WALLS, 2);
							state = MOVING;
							pathfind = true;
						}
						else
					...
					else
					{
						...
						{
							//soldiers in their area trying to deffend
							info.closest = FindNearestEntity(map_pos, fromPlayer1, entityType::SOLDIER, 2);
							if (info.closest->type < entityType::TANKMAN)
							{
								info.closest = nullptr;
								state = TROOP_IDLE;
								timer.Start();
...
void Tankman::MovementPathfind(Entity* target, pair <int, int> map_pos)
{
	bool north = true;
	bool south = true;
	bool west = true;
...
Entity* Tankman::FindEntityInAttackRange(pair <int, int> pos, bool fromplayer1, int attackrange, entityType desiredtype, int zone)
{
	// zone 0 is no zone
	// zone 1 ally zone
	// zone 2 enemy zone
...
Entity* Tankman::FindNearestEntity(pair <int, int> pos, bool fromplayer1, entityType desiredtype, int zones)
{
...
bool  Tankman::IsInAllyZone(pair <int, int > map_pos)
{
